...
export type TShareCoursePlacesInfo = TShareCourseInfo &
    TShareCoursePlace & {
        userId: string;
    };
 ...
 */
export const getAiKaKaoCourseAndId = async (
    tripId: string
): Promise<{
    aiCourseId: string;
    aiKaKaoCourse: Route[];
    aiCourse: IAiCourseResponse;
}> => {
    ...
    if (aiCourseIdArray.length !== 0) {
        ...
        return { aiCourseId, aiKaKaoCourse, aiCourse };
    }
